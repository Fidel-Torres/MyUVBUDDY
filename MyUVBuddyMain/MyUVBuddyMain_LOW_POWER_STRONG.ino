// LOW-POWER TEST BUILD: STRONG VERSION
// - Sleeps more aggressively, including during dashboard mode when idle.
// - Test carefully because button polling and BLE timing are more sensitive.
// =============================================================================
...
// Strong low-power guard. The MCU stays awake briefly after BLE/user events.
static unsigned long g_awake_until_ms = 0;
...
static void logBleStackPending()
{
  ...
  Serial.println("[POWER] Strong idle sleep enabled");
}
...
// =============================================================================
// Low-power idle control - STRONG version
// This version allows sleep even during BLE notification mode when no sensor
// sample or backlog replay is pending. It should save more power, but it is more
// aggressive and must be tested carefully with BLE reconnect, SETSED/RESET,
// button gestures, and haptic alerts.
// =============================================================================
...
static bool systemCanSleep()
{
  ...
  // Stay awake briefly after BLE/user events so commands and UI changes settle.
  if ((long)(millis() - g_awake_until_ms) < 0) return false;
...
static void send_pending_readings()
{
  ...
  if (sent_ok) {
    ...
    g_awake_until_ms = millis() + BACKLOG_SEND_SPACING_MS + 25;
